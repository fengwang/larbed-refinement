...
public:
    ...
    allocator(const self_type& rhs) : Allocator(rhs) 
    {
        operator = (rhs);
    }
    ...
    allocator() : buffer_(nullptr), items_(0)
    {}
...
template< typename Type, std::size_t N = 1, class Allocator = std::allocator<Type> >
struct matrix_allocator: allocator< Type, Allocator >
{
    typedef std::size_t                     size_type;
    typedef allocator< Type, Allocator >    host_allocator_type;

    explicit matrix_allocator(const size_type dims = 0) : host_allocator_type( dims ) {}

    template<typename Input_Iterator>
    matrix_allocator(Input_Iterator begin_, Input_Iterator end_) : host_allocator_type( begin_, end_ ) {}
